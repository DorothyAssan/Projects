...
SELECT country, product, AVG(price) AS average_price
FROM sales
GROUP BY country, product;

/*Total sales for each product where total sales > 5000 */
SELECT product, SUM(price) AS total_sales
FROM sales
GROUP BY product
HAVING total_sales > 2000;

/*Transactions by Country where number of transactions > 2 */
SELECT country, COUNT(*) AS transaction_count
FROM sales
GROUP BY country
HAVING transaction_count > 2;

/*Group data based on the price column*/
SELECT
  CASE
    WHEN price < 1500 THEN 'Low Price'
    WHEN price >= 1500 AND price < 3000 THEN 'Medium Price'
    WHEN price >= 3000 THEN 'High Price'
    ELSE 'Unknown'
  END AS price_group,
  COUNT(*) AS count
FROM sales
...
/*Selecting rows where country is United States and price < 3000*/
SELECT *
FROM sales
WHERE country = 'United States' AND price < 3000;

/* Selecting rows where either the country is 'United States' and the price is less than 3000 
or the country is 'New Zealand' and the product is 'Chair' */
SELECT *
FROM sales
WHERE (country = 'United States' AND price < 3000) OR (country = 'New Zealand' AND product = 'Chair');

/* Selecting rows where the state is 'Ontario' and the product is 'Chair' or 'Bed' */
SELECT *
FROM sales
WHERE state = 'Ontario' AND (product = 'Chair' OR product = 'Bed');
